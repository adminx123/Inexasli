...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateFormVisibility() {
        ...
        const isPartnered = /married|common_law|coupled|mfj|mfs/.test(fillingStatus);
        const hasDependants = numDependants > 0;

        const numDisabledDependantsInput = document.getElementById('numDisabledDependants');
        const numChildrenUnder19Input = document.getElementById('numChildrenUnder19');
        const numAdultDependantsInput = document.getElementById('numAdultDependants');
        const ageSpouseInput = document.getElementById('ageSpouse');
        const dependantsCheckbox = document.getElementById('dependants');
        ...
        const containers = {
            dependantFields: document.getElementById('dependantFields'),
            spouseFields: document.getElementById('spouseFields'),
            dependantsContainer: document.getElementById('dependantsContainer')
        };

        const labels = {
            numDisabledDependants: document.querySelector('label[for="numDisabledDependants"]'),
            numChildrenUnder19: document.querySelector('label[for="numChildrenUnder19"]'),
            numAdultDependants: document.querySelector('label[for="numAdultDependants"]'),
            ageSpouse: document.querySelector('label[for="ageSpouse"]'),
            dependants: document.querySelector('label[for="dependants"]')
        };

        if (country && (subregion || country === 'OTHER')) {
            filingStatusSection.style.display = 'block';
        } else {
            filingStatusSection.style.display = 'none';
        }
        ...
        if (hasDependants) {
            containers.dependantFields.style.display = 'block';
            containers.dependantsContainer.style.display = 'block';
            numDisabledDependantsInput.style.display = 'block';
            numChildrenUnder19Input.style.display = 'block';
            numAdultDependantsInput.style.display = 'block';
            dependantsCheckbox.style.display = 'inline-block';
            labels.numDisabledDependants.style.display = 'block';
            labels.numChildrenUnder19.style.display = 'block';
            labels.numAdultDependants.style.display = 'block';
            labels.dependants.style.display = 'block';
        } else {
            containers.dependantFields.style.display = 'none';
            containers.dependantsContainer.style.display = 'none';
            numDisabledDependantsInput.style.display = 'none';
            numChildrenUnder19Input.style.display = 'none';
            numAdultDependantsInput.style.display = 'none';
            dependantsCheckbox.style.display = 'none';
            labels.numDisabledDependants.style.display = 'none';
            labels.numChildrenUnder19.style.display = 'none';
            labels.numAdultDependants.style.display = 'none';
            labels.dependants.style.display = 'none';
            setLocal('numDisabledDependants', '', 365);
            setLocal('numChildrenUnder19', '', 365);
            setLocal('numAdultDependants', '', 365);
            setLocal('dependants', 'unChecked', 365);
            dependantsCheckbox.checked = false;
        }

        if (isPartnered) {
            containers.spouseFields.style.display = 'block';
            ageSpouseInput.style.display = 'block';
            labels.ageSpouse.style.display = 'block';
        } else {
            containers.spouseFields.style.display = 'none';
            ageSpouseInput.style.display = 'none';
            labels.ageSpouse.style.display = 'none';
            setLocal('ageSpouse', '', 365);
        }

        const isValid = country && (subregion || country === 'OTHER') && fillingStatus;
        proceedButton.classList.toggle('enabled', isValid);
        proceedButton.classList.toggle('disabled', !isValid);
        proceedButton.classList.toggle('pulse', isValid);
    }
